...
class OpenAILLM(LLM):
    ...
    async def invoke(
        ...
        """使用异步OpenAI发起块响应(该步骤可以切换成流式响应)"""
        try:
            # 1.检测是否传递了工具列表
            if tools:
                logger.info(f"调用OpenAI客户端向LLM发起请求并携带工具信息:{self.model_name}")
                ...
                logger.info(f"调用OpenAI客户端向LLM发起请求未携带工具:{self.model_name}")
                response = await self._client.chat.completions.create(
                    model=self.model_name,
                    temperature=self.temperature,
                    max_tokens=self.max_tokens,
            ...
            logger.error(f"调用OpenAI客户端发送错误：{str(e)}")
